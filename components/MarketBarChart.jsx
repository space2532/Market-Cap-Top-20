...
const MarketBarChart = ({ data, onCompanyClick }) => {
  ...
  useEffect(() => {
    // Keep empty observer to trigger initial render without enforcing container-driven scaling
    setWidth(prev => prev); // no-op to satisfy state usage
  }, []);
  ...
  useEffect(() => {
    ...
    const logicalWidth = 1200; // fixed logical width to keep constant aspect ratio
    const renderedWidth = logicalWidth;
    const innerWidth = Math.max(0, logicalWidth - margin.left - margin.right);
    
    const svg = d3.select(svgRef.current)
      .attr('viewBox', `0 0 ${renderedWidth} ${svgHeight}`)
    ...
    const domainMax = maxMarketCap; // base domain on actual max

    const xScale = d3.scaleLinear().domain([0, domainMax]).nice().range([0, innerWidth]);
    ...
    const bars = g.selectAll('.bar')
      ...
      .join(
        enter => {
          ...
          const rightPad = 12; // avoid overflow outside inner plot
          enterGroups.append('image').attr('class', 'company-logo').attr('href', d => d.logo_url || '').attr('x', d => Math.min(xScale(d.market_cap_usd) - 45, innerWidth - 45 - rightPad)).attr('y', yScale.bandwidth() / 2 - 20).attr('width', 40).attr('height', 40);
          enterGroups.append('text').attr('class', 'market-cap-value').attr('x', d => Math.min(xScale(d.market_cap_usd) + 15, innerWidth - rightPad)).attr('y', yScale.bandwidth() / 2).attr('dy', '0.35em').attr('font-size', '14px').attr('font-weight', '500').attr('fill', '#6B7280').text(d => d.market_cap_display);
          ...
          enterGroups.selectAll('.company-logo').transition().duration(800).ease(d3.easeCubicInOut).delay(300).attr('x', d => Math.min(xScale(d.market_cap_usd) - 45, innerWidth - 45 - 12));
          enterGroups.selectAll('.market-cap-value').transition().duration(800).ease(d3.easeCubicInOut).delay(300).attr('x', d => Math.min(xScale(d.market_cap_usd) + 15, innerWidth - 12));
          return enterGroups;
        },
        update => {
          update
          ...
          update.selectAll('.company-logo').transition().duration(800).ease(d3.easeCubicInOut).attr('href', d => d.logo_url || '').attr('x', d => Math.min(xScale(d.market_cap_usd) - 45, innerWidth - 45 - 12));
          update.selectAll('.market-cap-value').transition().duration(800).ease(d3.easeCubicInOut).attr('x', d => Math.min(xScale(d.market_cap_usd) + 15, innerWidth - 12)).text(d => d.market_cap_display);
          return update;
        },
  ...
  return (
    <div ref={containerRef} className="w-full bg-white rounded-lg border border-gray-200 shadow-sm">
      <svg ref={svgRef} className="w-full h-auto" style={{ display: 'block' }} />
    </div>
  );
};
